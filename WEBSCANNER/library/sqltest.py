		...
		if not known_url.get(url) and n_host == base_host:
			try:
				...
				if response.status == 200:
					...
					soup = BeautifulSoup(content.decode('ascii', 'ignore'))
...
browser = httplib2.Http(disable_ssl_certificate_validation=True)

sys.stdout.write("[+] Spidering the URL for interesting Links :")
spider(start_url)

sys.stdout.write("\n[-] Done Spidering. \n")

sys.stdout.write("\n[+] Attacking found Links\n")
